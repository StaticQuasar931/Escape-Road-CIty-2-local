...
/* Patch document element lookups so Unity code never crashes
   when something like document[obfuscatedName](...) returns null */
(function(){
    var REAL_getElementById = document.getElementById.bind(document);
    var REAL_querySelector  = document.querySelector.bind(document);
    var dummyById = Object.create(null);
    var dummyBySelector = Object.create(null);
    ...
    document.getElementById = function(id){
        ...
        if (dummyById[id]) return dummyById[id];
    ...
    document.querySelector = function(sel){
        var el = REAL_querySelector(sel);
        if (el) return el;
        if (!sel) return null;

        if (dummyBySelector[sel]) return dummyBySelector[sel];

        var dummy = document.createElement("div");
        dummy.setAttribute("data-dummy-selector", sel);
        dummy.style.display = "none";
        dummyBySelector[sel] = dummy;
...
/* IMPORTANT: Signature must accept one parameter so Unity does not complain */
window.GmSoft.SetParam = function(param){
    console.log("GmSoft.SetParam (Unity-safe)", param, arguments);
    return true;
};

/* Also expose a global SetParam alias in case the game calls it directly */
window.SetParam = window.GmSoft.SetParam;

window.GmSoft.GetParam = function(key){
    console.log("GmSoft.GetParam (Unity-safe)", key);
    return "";
};
